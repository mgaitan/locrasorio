...
function updateCountdown() {
  ...
  if (el) {
    ...
    el.querySelector("[data-minutes]").textContent = String(minutes).padStart(
      2,
      "0",
    );
  }
...
guestsInput?.addEventListener("input", () => {
  ...
  checkLocroWarning(Number(guestsInput.value));
});
guestsSlider?.addEventListener("input", () => {
  setGuests(guestsSlider.value);
  checkLocroWarning(Number(guestsSlider.value));
});
guestsDec?.addEventListener("click", () => {
  const n = Number(guestsInput.value) - 5;
  setGuests(n);
  checkLocroWarning(n);
});
...
if (calLink) {
  ...
  const params = new URLSearchParams({
    action: "TEMPLATE",
    text: "El Locrasorio 🍲 de Nati & Martín",
    dates: "20260523T150000Z/20260523T220000Z",
    details:
      "Un locraso y un casorio en el mismo evento. \n\nhttps://locrasorio.pages.dev",
    location: "El Casco Eventos, Alta Gracia, Córdoba, Argentina",
  });
